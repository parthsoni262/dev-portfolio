...
var sortable = (function () {
  ...
  function addData(element, key, value) {
    if (value === undefined) {
      return (
        element && element.h5s && element.h5s.data && element.h5s.data[key]
      );
    } else {
  ...
  function removeEventListener(element, eventName) {
    ...
    element.removeEventListener(
      eventName,
      store(element).getData("event" + eventName)
    );
  ...
  var getIndex = function (element, elementList) {
    if (
      ...
      !(
        ...
        elementList instanceof HTMLCollection ||
        elementList instanceof Array
      )
    ) {
  ...
  var serialize = function (
    ...
    if (
      !(sortableContainer instanceof HTMLElement) ||
      !sortableContainer.isSortable === true
    ) {
      throw new Error(
    ...
    if (
      typeof customItemSerializer !== "function" ||
      typeof customContainerSerializer !== "function"
    ) {
      throw new Error(
    ...
    return (
      items
        ...
        .filter(function (item) {
          return (
            item.querySelector(selector) instanceof HTMLElement ||
            (item.shadowRoot &&
              item.shadowRoot.querySelector(selector) instanceof HTMLElement)
          );
        })
        ...
        .map(function (item) {
          return (
            ...
            (item.shadowRoot && item.shadowRoot.querySelector(selector))
          );
        })
    );
  };
  ...
  var setDragImage = function (event, draggedElement, customDragImage) {
    ...
    if (event.dataTransfer && event.dataTransfer.setDragImage) {
      ...
      if (
        !(dragImage.element instanceof HTMLElement) ||
        typeof dragImage.posX !== "number" ||
        typeof dragImage.posY !== "number"
      ) {
        throw new Error(
  ...
  var listsConnected = function (destination, origin) {
    ...
    if (destination.isSortable === true) {
      ...
      if (acceptFrom !== null) {
        return (
          acceptFrom !== false &&
          acceptFrom.split(",").filter(function (sel) {
      ...
      if (
        ...
        store(destination).getConfig("connectWith") !== null
      ) {
        return (
          store(destination).getConfig("connectWith") ===
          store(origin).getConfig("connectWith")
        );
      }
    }
    return false;
  ...
  function throttle(fn, threshold) {
    ...
    return function () {
      ...
      if (
        ...
        now - lastEventTimestamp >= threshold
      ) {
  ...
  var enableHoverClass = function (sortableContainer, enable) {
    if (typeof store(sortableContainer).getConfig("hoverClass") === "string") {
      var hoverClasses_1 = store(sortableContainer)
          ...
          throttle(function (event) {
            ...
            if (event.buttons === 0) {
              filter(
                ...
                store(sortableContainer).getConfig("items")
  ...
  function findDragElement(sortableElement, element) {
    ...
    var itemlist = items.filter(function (ele) {
      return (
        ...
        (ele.shadowRoot && ele.shadowRoot.contains(element))
      );
    });
  ...
  function sortable(sortableElements, options) {
    ...
    sortableElements.forEach(function (sortableElement) {
      ...
      ["connectWith", "disableIEFix"].forEach(function (configKey) {
        if (
          Object.prototype.hasOwnProperty.call(options, configKey) &&
          options[configKey] !== null
        ) {
          console.warn(
      ...
      // add placeholder
      store(sortableElement).placeholder = makePlaceholder(
        sortableElement,
        customPlaceholder,
      ...
      addEventListener(sortableElement, "dragstart", function (e) {
        ...
        if (
          (options.handle && !target.matches(options.handle)) ||
        ...
        // grab values
        originItemsBeforeUpdate = filter(
          sortableContainer.children,
          options.items
        );
        ...
        sortableContainer.dispatchEvent(
      ...
      addEventListener(sortableElement, "dragenter", function (e) {
        ...
        if (sortableContainer && sortableContainer !== previousContainer) {
          destinationItemsBeforeUpdate = filter(
            sortableContainer.children,
            addData(sortableContainer, "items")
          ).filter(function (item) {
      ...
      addEventListener(sortableElement, "dragend", function (e) {
        ...
        if (
          dragging.getAttribute("aria-copied") === "true" &&
        ...
        var visiblePlaceholder = Array.from(stores.values())
          .map(function (data) {
      ...
      addEventListener(sortableElement, "drop", function (e) {
        ...
        var originItems = filter(
          originContainer.children,
          options.items
        ).filter(function (item) {
          return item !== placeholder;
        });
        var destinationContainer =
          this.isSortable === true ? this : this.parentElement;
        var destinationItems = filter(
          destinationContainer.children,
          addData(destinationContainer, "items")
        ).filter(function (item) {
          return item !== placeholder;
        });
        var destinationElementIndex = getIndex(
          dragging,
          Array.from(dragging.parentElement.children).filter(function (item) {
            return item !== placeholder;
          })
        );
         ...
         */
        if (
          originElementIndex !== destinationElementIndex ||
          originContainer !== destinationContainer
        ) {
          sortableElement.dispatchEvent(
      ...
      var debouncedDragOverEnter = debounce(function (
        sortableElement,
        element,
        pageX,
        pageY
        ...
        if (options.forcePlaceholderSize) {
          store(sortableElement).placeholder.style.height =
            draggingHeight + "px";
        ...
        if (Array.from(sortableElement.children).indexOf(element) > -1) {
          ...
          var placeholderIndex = getIndex(
            store(sortableElement).placeholder,
          ...
          if (thisHeight > draggingHeight || thisWidth > draggingWidth) {
            ...
            if (
              ...
              ((options.orientation === "vertical" &&
                pageY > offsetTop + thisHeight - deadZoneVertical) ||
                (options.orientation === "horizontal" &&
                  pageX > offsetLeft + thisWidth - deadZoneHorizontal))
            ) {
              return;
            }
          }
          ...
          try {
            var elementMiddleVertical =
              offset(element).top + element.offsetHeight / 2;
            var elementMiddleHorizontal =
              offset(element).left + element.offsetWidth / 2;
            placeAfter =
              (options.orientation === "vertical" &&
                pageY >= elementMiddleVertical) ||
              (options.orientation === "horizontal" &&
                pageX >= elementMiddleHorizontal);
          } catch (e) {
          ...
          if (
            placeholders.indexOf(element) === -1 &&
      ...
      var onDragOverEnter = function (e) {
        ...
        var sortableElement =
          element.isSortable === true ? element : findSortable(element, e);
        element = findDragElement(sortableElement, element);
        if (
          !dragging ||
          !listsConnected(sortableElement, dragging.parentElement) ||
          addData(sortableElement, "_disabled") === "true"
        ) {
          return;
        }
        ...
        if (
          ...
          filter(sortableElement.children, addData(sortableElement, "items"))
            .length > parseInt(options.maxItems) &&
          dragging.parentElement !== sortableElement
        ) {
          return;
        }
        e.preventDefault();
        ...
        e.dataTransfer.dropEffect =
          store(sortableElement).getConfig("copy") === true ? "copy" : "move";
        debouncedDragOverEnter(sortableElement, element, e.pageX, e.pageY);
      };
      addEventListener(
        listItems.concat(sortableElement),
        "dragover",
        onDragOverEnter
      );
      addEventListener(
